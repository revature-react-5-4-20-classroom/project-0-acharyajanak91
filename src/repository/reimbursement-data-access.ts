...
export async function getReimbursementById(id:number):Promise<Reimbursement[]> {
    ...
    try{
        let result: QueryResult;
        result=await client.query(
           `SELECT * FROM Reimbursement WHERE users.id=$1;`,[id]);
        return result.rows;
           // return result.rows.map((u)=>{
...
// export async function addNewReimbursement(reimburseemnt:Reimbursement) : Promise<Reimbursement> {
//     let client: PoolClient=await connectionPool.connect();
//     try{
//         const roleIdResult : QueryResult=await client.query(
//             `SELECT * FROM reimbursement WHERE reimbursement.author=$1 or reimbursement.`,[reimburseemnt.author]
//         );
